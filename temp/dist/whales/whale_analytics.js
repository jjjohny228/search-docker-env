...
class WhaleAnalytics {
    ...
    computeAllMetrics(whaleId) {
        ...
        logs_1.logger.debug({ whaleId, score: score.overall, style }, 'Computed whale metrics');
    }
    /* ━━━━━━━━━━━━━━ Settlement Ledger (FIFO / AVG) ━━━━━━━━━━━━━━ */
    buildSettlementLedger(whaleId) {
    ...
    processLotGroup(whaleId, trades, method) {
        const lots = [];
        let lotCounter = 0;
                ...
                if (method === 'FIFO') {
                    ...
                    if (lots.length > 0) {
                        ...
                        for (const lot of lots)
                            lot.qty *= factor;
                        // Remove empty lots
                        while (lots.length > 0 && lots[0].qty < 0.0001)
                            lots.shift();
                    }
                }
            }
    ...
    computeDailyMetrics(whaleId) {
        ...
        for (const [date, trades] of byDate) {
            ...
            const dateEntries = settlementEntries.filter((e) => e.closeTs && e.closeTs.slice(0, 10) === date);
            ...
            const holdMinutes = dateEntries
                .filter((e) => e.closeTs)
                .map((e) => (new Date(e.closeTs).getTime() - new Date(e.openTs).getTime()) / 60000);
    ...
    computeTimingAnalysis(whaleId) {
        ...
        for (const windowMinutes of windows) {
            ...
            for (let i = 0; i < allTrades.length; i++) {
                ...
                const windowEnd = new Date(trade.ts).getTime() + windowMinutes * 60000;
                // Find trades in same market within window
                const laterTrades = allTrades.filter((t) => t.marketId === trade.marketId &&
                ...
                const moveBps = ((refPrice - trade.price) / trade.price) * 10000;
    ...
    computeScore(whaleId) {
        ...
        const profitability = Math.min(100, Math.max(0, (winRate * 60) + (Math.min(roi, 0.5) / 0.5 * 40)));
        ...
        const marketQuality = Math.min(100, Math.max(0, Math.min(distinctMarkets, 20) / 20 * 50 + winRate * 50));
        ...
        let overall = profitability * w.profitability +
        ...
        if (whale?.dataIntegrity === 'BACKFILLING')
            dataIntegrityModifier = 0.7;
        else if (whale?.dataIntegrity === 'DEGRADED')
            dataIntegrityModifier = 0.85;
        overall *= dataIntegrityModifier;
        // Provisional cap
        if (provisional)
            overall = Math.min(overall, this.config.provisionalMaxScore);
    ...
    computeConsistencyScore(dailyReturns) {
    ...
    classifyStyle(whaleId) {
        ...
        if (avgHold < 30 && avgTrades > 10)
            return 'SCALPER';
        // Accumulator: few large trades, long hold
        if (avgHold > 1440 && avgTrades < 5 && avgVolume > 5000)
            return 'ACCUMULATOR';
        ...
        if (avgHold >= 30 && avgHold <= 1440 && winRate > 0.5)
            return 'MOMENTUM';
        return 'UNKNOWN';
    }
    ...
    /* ━━━━━━━━━━━━━━ Helpers ━━━━━━━━━━━━━━ */
    getAllTradesSorted(whaleId) {
        // Fetch all trades (paginate via large limit) — DB returns DESC, we need ASC
        const trades = this.db.getWhaleTrades(whaleId, { limit: 100000 });
        return trades.reverse();
    }
}
exports.WhaleAnalytics = WhaleAnalytics;
