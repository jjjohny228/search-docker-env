...
exports.WhaleCandidates = void 0;
const logs_1 = require("../reporting/logs");
class WhaleCandidates {
    constructor(db, config, clobApi) {
        this.running = false;
        this.scanTimer = null;
        this.db = db;
        this.config = config;
    ...
    start() {
        if (this.running)
            return;
        this.running = true;
        logs_1.logger.info('WhaleCandidates discovery started');
    ...
    stop() {
        ...
        logs_1.logger.info('WhaleCandidates discovery stopped');
    }
    /* ━━━━━━━━━━━━━━ Main scan cycle ━━━━━━━━━━━━━━ */
    async scanCycle() {
        if (!this.running)
        ...
        try {
            ...
            for (const [address, stats] of addressStats) {
                ...
                if (stats.volumeUsd24h < this.config.candidateMinVolumeUsd24h)
                    continue;
                if (stats.trades24h < this.config.candidateMinTrades24h)
                    continue;
            ...
            if (candidatesUpserted > 0) {
                logs_1.logger.info({ candidatesUpserted }, 'Whale candidate scan complete');
            }
            ...
            logs_1.logger.error({ err }, 'Whale candidate scan error');
        }
    }
    /* ━━━━━━━━━━━━━━ Fetch recent trades ━━━━━━━━━━━━━━ */
    async fetchRecentTrades() {
        ...
        try {
            ...
            logs_1.logger.debug({ status: res.status }, 'CLOB trades unavailable, falling back to data-api');
        }
        catch {
            logs_1.logger.debug('CLOB trades fetch error, falling back to data-api');
        }
    ...
    async fetchFromDataApi() {
        ...
        try {
            ...
            const marketsRes = await fetch(`${gammaApi}/markets?active=true&closed=false&limit=10&order=volume24hr&ascending=false`);
            if (!marketsRes.ok)
                return [];
            ...
            if (allTrades.length > 0) {
                logs_1.logger.info({ trades: allTrades.length }, 'Candidate scan: fetched trades via data-api fallback');
            }
            return allTrades;
        }
        catch (err) {
            logs_1.logger.warn({ err }, 'Candidate scan: data-api fallback error');
            return [];
        }
    }
    /* ━━━━━━━━━━━━━━ Aggregation ━━━━━━━━━━━━━━ */
    aggregateByAddress(trades) {
        const stats = new Map();
        for (const t of trades) {
            ...
            for (const rawAddr of addresses) {
                ...
                if (t.match_time < s.firstSeen)
                    s.firstSeen = t.match_time;
                if (t.match_time > s.lastSeen)
                    s.lastSeen = t.match_time;
                if (t.side === 'BUY')
                    s.buys++;
                else
                    s.sells++;
    ...
    suggestTags(stats) {
        const tags = [];
        if (stats.volumeUsd24h > 100000)
            tags.push('high_volume');
        if (stats.avgTradeSize > 5000)
            tags.push('large_trades');
        if (stats.buySellRatio > 3)
            tags.push('aggressive_buyer');
        if (stats.buySellRatio < 0.33)
            tags.push('aggressive_seller');
        if (stats.distinctMarkets > 10)
            tags.push('diversified');
        if (stats.distinctMarkets === 1)
            tags.push('concentrated');
        return tags;
    }
    /* ━━━━━━━━━━━━━━ Auto-track ━━━━━━━━━━━━━━ */
    async autoTrackTopCandidates() {
        ...
        for (const c of candidates) {
            if (c.rankScore >= 60) {
                ...
                logs_1.logger.info({ address: c.address.slice(0, 10) + '...', rankScore: c.rankScore }, 'Auto-tracked whale candidate');
            }
        }
    }
}
exports.WhaleCandidates = WhaleCandidates;
