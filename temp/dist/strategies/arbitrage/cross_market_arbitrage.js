...
exports.CrossMarketArbitrageStrategy = void 0;
...
class CrossMarketArbitrageStrategy extends strategy_interface_1.BaseStrategy {
    constructor() {
        super(...arguments);
        this.name = 'cross_market_arbitrage';
        this.cooldownMs = 60000; // 1-min cooldown per market (arbs are time-sensitive)
        this.positions = [];
    ...
    generateSignals() {
        ...
        for (const [, market] of this.markets) {
            ...
            const feeCost = (TAKER_FEE_BPS / 10000) * 2; // fee on each side
            if (totalPrice < 1 - 0.005) {
                ...
                if (netEdge * 10000 < MIN_NET_EDGE_BPS)
                    continue;
                ...
                if (netEdge > 0.015) {
                    const otherSide = cheaperSide === 'YES' ? 'NO' : 'YES';
                    signals.push({
                        marketId: market.marketId,
                        outcome: otherSide,
                        side: 'BUY',
            ...
            if (totalPrice > 1 + 0.005) {
                ...
                if (netEdge * 10000 < MIN_NET_EDGE_BPS)
                    continue;
                const conf = this.computeConfidence(netEdge, market);
                const expensiveSide = yesPrice >= noPrice ? 'YES' : 'NO';
                signals.push({
                    marketId: market.marketId,
        ...
        for (const [, group] of eventGroups) {
            ...
            if (deviation > 0.03 && deviation < 0.25) {
                ...
                const sorted = [...group].sort((a, b) => a.outcomePrices[0] - b.outcomePrices[0]);
                if (totalYes < 1) {
                    // Underpriced: buy the cheapest YES
                    const cheapest = sorted[0];
                    if (!this.passesFilters(cheapest, now))
                        continue;
                    const netEdge = deviation / group.length - (TAKER_FEE_BPS / 10000);
                    if (netEdge * 10000 < MIN_NET_EDGE_BPS)
                        continue;
                    signals.push({
                    ...
                    if (!this.passesFilters(expensive, now))
                        continue;
                    const netEdge = deviation / group.length - (TAKER_FEE_BPS / 10000);
                    if (netEdge * 10000 < MIN_NET_EDGE_BPS)
                        continue;
    ...
    computeConfidence(netEdge, market) {
        ...
        const volumeScore = Math.min(1, market.volume24h / 20000);
...
exports.CrossMarketArbitrageStrategy = CrossMarketArbitrageStrategy;
