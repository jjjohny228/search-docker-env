...
class MomentumStrategy extends strategy_interface_1.BaseStrategy {
    constructor() {
        super(...arguments);
        this.name = 'momentum';
    ...
    initialize(context) {
        ...
        logs_1.logger.info({ strategy: this.name }, 'Momentum strategy initialised');
    }
    onMarketUpdate(data) {
    ...
    generateSignals() {
        ...
        for (const [marketId, market] of this.markets) {
            ...
            if (this.managedPositions.has(marketId))
                continue;
            // Max positions check
            if (this.managedPositions.size >= MAX_POSITIONS)
                break;
            // Per-market cooldown
            ...
            if (Math.abs(dayChg) >= MIN_DAY_CHANGE || Math.abs(weekChg) >= MIN_WEEK_CHANGE) {
                ...
                if (rsiHist && rsiHist.length >= RSI_PERIOD + 1) {
                    ...
                    if (isBullish && rsi > RSI_OVERBOUGHT)
                        continue;
                    if (!isBullish && rsi < RSI_OVERSOLD)
                        continue;
                }
                const edgeRaw = Math.min(0.06, absDayChg * 0.4 + absWeekChg * 0.15);
                const confidence = Math.min(0.80, 0.35
                    + Math.min(0.20, absDayChg * 2.5)
                    + Math.min(0.10, absWeekChg * 0.8)
                    + (market.volume24h > 5000 ? 0.05 : 0)
            ...
            const crossBps = Math.abs(shortMa - medMa) / Math.max(medMa, 0.001) * 10000;
            if (crossBps < MA_CROSS_MIN_BPS)
                continue;
            if (adx < ADX_MIN)
                continue;
            ...
            if (shortAboveMed > 0 && medAboveLong > 0 && rsi > 30 && rsi < RSI_OVERBOUGHT && volConfirming) {
                ...
                const confidence = Math.min(0.80, 0.30 + adx * 0.25 + (crossBps / 200) * 0.15 + (volConfirming ? 0.1 : 0)
                    + (market.spread < 0.03 ? 0.05 : 0));
                if (confidence >= 0.42) {
        ...
        if (this.scanCount % 12 === 0) {
            console_log_1.consoleLog.info('STRATEGY', `[momentum] Scan #${this.scanCount}: ${totalMarkets} mkts, gamma=${gammaHits}, micro=${microTrendHits}, signals=${signals.length}, open=${this.managedPositions.size}`, {
    ...
    sizePositions(signals) {
        ...
        for (const signal of signals) {
            if (orders.length >= slotsAvailable)
                break;
            if (deployedThisTick >= maxTickDeploy)
                break;
            if (this.managedPositions.has(signal.marketId))
                continue;
            ...
            const maxDollars = Math.min(available * Math.min(halfKelly, MAX_CAPITAL_PCT), maxTickDeploy - deployedThisTick);
            if (maxDollars < 1)
                continue;
            const size = Math.min(Math.max(1, Math.floor(maxDollars / safePrice)), MAX_SHARES);
            const cost = size * safePrice;
            if (deployedThisTick + cost > maxTickDeploy)
                continue;
            if (cost > available - deployedThisTick)
                continue;
    ...
    managePositions() {
        ...
        for (const [marketId, pos] of this.managedPositions) {
            ...
            const pnlBps = pos.direction === 'BULL'
                ? ((currentPrice - pos.entryPrice) / Math.max(pos.entryPrice, 0.001)) * 10000
                : ((pos.entryPrice - currentPrice) / Math.max(pos.entryPrice, 0.001)) * 10000;
            ...
            if (exitReason) {
                ...
                logs_1.logger.info({ strategy: this.name, marketId, outcome: pos.outcome, reason: exitReason, pnlBps: pnlBps.toFixed(0) }, `MOMENTUM exit: ${exitReason}`);
            }
        }
    }
    ...
    computeAdx(prices) {
        ...
        if (actualMoves < 2)
            return 0; // relaxed: only 2 directional changes needed
