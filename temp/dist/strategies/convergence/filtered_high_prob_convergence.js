...
exports.FilteredHighProbConvergenceStrategy = void 0;
const strategy_interface_1 = require("../strategy_interface");
const trade_history_1 = require("../../data/trade_history");
...
const DEFAULTS = {
    ...
    min_depth_usd_within_1pct: 2000,
    min_imbalance: 0.10,
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   ...
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
class FilteredHighProbConvergenceStrategy extends strategy_interface_1.BaseStrategy {
    constructor() {
        super(...arguments);
        this.name = 'filtered_high_prob_convergence';
        /* ── Configuration ── */
        this.cfg = { ...DEFAULTS };
        ...
        this.rateBucket = { timestamps: [], cancels: 0 };
        ...
        this.tradeHistory = new trade_history_1.TradeHistory();
        /* ── Increase cooldown for this strategy (5 minutes) ── */
        this.cooldownMs = 300000;
        /* ── Trailing-stop / partial-exit thresholds (bps) ── */
        this.TRAILING_STOP_PULLBACK_BPS = 80;
        this.PARTIAL_EXIT_1_BPS = 100;
        this.PARTIAL_EXIT_2_BPS = 160;
    }
    /* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
       Lifecycle
       ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
    initialize(context) {
        ...
        logs_1.logger.info({ strategy: this.name, cfg: this.cfg }, 'FilteredHighProbConvergence initialised');
    }
    /* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    generateSignals() {
        if (!this.cfg.enabled)
            return [];
        /* ── Drawdown guard ── */
        if (this.isDailyLossBreached() || this.isWeeklyDrawdownBreached())
            return [];
        const signals = [];
        let evaluated = 0;
        ...
        if (evaluated > 0) {
            logs_1.logger.info({ strategy: this.name, evaluated, passed: signals.length }, `Convergence scan: ${signals.length}/${evaluated} markets passed all 8 filters`);
        }
        return signals;
    }
    /* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    sizePositions(signals) {
        ...
        for (const order of cooldownFiltered.slice(0, slotsAvailable)) {
            ...
            if (!this.checkRateLimit()) {
                logs_1.logger.warn({ strategy: this.name }, 'Order rate limit reached – pausing entries');
                break;
            }
            ...
            if (mlePct > this.cfg.max_market_mle_pct)
                continue;
            /* Total MLE across all managed positions */
            const totalMle = this.managedPositions.reduce((sum, p) => sum + p.costBasis, 0) + mle;
            if (totalMle / capital > this.cfg.max_total_mle_pct)
                continue;
            /* Cluster exposure check */
            if (!this.checkClusterExposure(market, positionUsd))
                continue;
            sized.push({
                ...order,
    ...
    notifyFill(order) {
        if (order.strategy !== this.name)
            return;
        const market = this.markets.get(order.marketId);
        const score = market ? this.computeSetupScore(market) : { value: 0.5 };
    ...
    managePositions() {
        ...
        for (const pos of this.managedPositions) {
            ...
            const entryBps = ((currentMid - pos.entryPrice) / pos.entryPrice) * 10000;
            ...
            if (!exitReason && market.endDate) {
                const daysLeft = (new Date(market.endDate).getTime() - now) / 86400000;
                const spreadBps = (market.spread / market.midPrice) * 10000;
        ...
        for (const closed of toClose) {
            ...
            if (idx >= 0)
                this.managedPositions.splice(idx, 1);
        }
    }
    /* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
       FILTER PIPELINE — 8 filters
       ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
    evaluateMarket(marketId, m) {
        const filterNames = [
        ...
        for (let i = 0; i < filters.length; i++) {
            const f = filters[i];
            if (!f.pass) {
                logs_1.logger.debug({ strategy: this.name, marketId, filter: filterNames[i], reason: f.reason }, `Filter ${filterNames[i]} rejected market`);
                return null;
            }
        }
        /* All filters passed → decide direction */
        const impliedProb = m.midPrice;
        const outcome = impliedProb >= 0.5 ? 'YES' : 'NO';
    ...
    /* ─── C) Spread Filter ─── */
    filterSpread(m) {
        const spreadBps = (m.spread / Math.max(0.001, m.midPrice)) * 10000;
    ...
    filterTimeToResolution(m) {
        if (!m.endDate) {
            return { pass: false, reason: 'no endDate – skipping unknown horizon' };
        }
        const daysLeft = (new Date(m.endDate).getTime() - Date.now()) / 86400000;
        if (daysLeft <= 0) {
    ...
    filterAntiChasing(m) {
        ...
        if (!clobId)
            return { pass: true }; // can't check, allow
        const history = this.priceCache.get(clobId) ?? [];
        if (history.length < 4)
            return { pass: true }; // not enough data yet
        const lookbackMs = this.cfg.spike_lookback_minutes * 60000;
    ...
    filterFlowPressure(m) {
        ...
        if (clobId) {
            const history = this.priceCache.get(clobId) ?? [];
            const cutoff = Date.now() - this.cfg.flow_lookback_minutes * 60000;
            const recent = history.filter((p) => p.timestamp >= cutoff);
            if (recent.length >= 2) {
    ...
    filterVolumeTrend(m) {
        ...
        if (volHistory.length < 4)
            return { pass: true }; // not enough data
    ...
    computeSetupScore(m) {
        ...
        if (m.endDate) {
            const daysLeft = Math.max(0.1, (new Date(m.endDate).getTime() - Date.now()) / 86400000);
    ...
    checkRateLimit() {
        const now = Date.now();
        this.rateBucket.timestamps = this.rateBucket.timestamps.filter((t) => now - t < 60000);
    ...
    resetDrawdownCounters() {
        ...
        if (now - this.weekStart > 7 * 86400000) {
            this.weeklyPnl = 0;
            this.weekStart = now;
        }
    }
...
exports.FilteredHighProbConvergenceStrategy = FilteredHighProbConvergenceStrategy;
