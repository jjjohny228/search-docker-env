...
exports.AiForecastStrategy = void 0;
...
class AiForecastStrategy extends strategy_interface_1.BaseStrategy {
    constructor() {
        super(...arguments);
        this.name = 'ai_forecast';
        this.cooldownMs = 150000; // 2.5 min
    ...
    generateSignals() {
        ...
        for (const [marketId, market] of this.markets) {
            ...
            const confidence = Math.min(MAX_CONFIDENCE, 0.3 + agreementRatio * 0.3 + avgStrength * 0.2 + regimeBonus);
            signals.push({
                marketId,
                outcome,
                side: 'BUY',
    ...
    factorVolumePriceDivergence(prices, volumes) {
    ...
    factorVolatility(prices, regime) {
    ...
    /* ── Run all factors ────────────────────────────────────────── */
    runFactors(_marketId, market, prices, volumes, regime) {
        return [
    ...
    passesFilters(market) {
        if (market.volume24h < MIN_VOLUME)
            return false;
        if (market.liquidity < MIN_LIQUIDITY)
            return false;
        const yesPrice = market.outcomePrices[0] ?? 0.5;
        if (yesPrice < PRICE_FLOOR || yesPrice > PRICE_CEILING)
            return false;
        return true;
    }
}
exports.AiForecastStrategy = AiForecastStrategy;
