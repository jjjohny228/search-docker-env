...
const paper_wallet_1 = require("../wallets/paper_wallet");
const market_fetcher_1 = require("../data/market_fetcher");
const dashboard_api_1 = require("./dashboard_api");
const registry_1 = require("../strategies/registry");
const logs_1 = require("./logs");
const console_log_1 = require("./console_log");
const copy_trade_strategy_1 = require("../strategies/copy_trading/copy_trade_strategy");
function buildWalletDetail(wallet, trades, marketPrices) {
    ...
    return [
        {
            ...
            description: 'Detects pricing inconsistencies between related prediction markets. If "Will X happen by March?" is priced higher than "Will X happen by June?", that is an arbitrage opportunity.',
            howItWorks: [
        ...
        },
        {
            ...
            category: 'Arbitrage',
            riskLevel: 'Low',
            description: 'Identifies outcomes whose probabilities don\'t sum correctly. In a binary market (Yes/No), prices should sum to ~$1.00. When they don\'t, there is a risk-free profit.',
            howItWorks: [
        ...
        },
        {
            ...
            description: 'Uses web research and data analysis to estimate the true probability of an event, then trades when the market price diverges significantly from the AI estimate.',
            howItWorks: [
        ...
        },
        {
            ...
            riskLevel: 'Medium',
            description: 'Places bid and ask orders around estimated fair value to capture the spread. Profits from the difference between buy and sell prices.',
            howItWorks: [
        ...
        },
        {
            ...
            riskLevel: 'High',
            description: 'Trades in the direction of recent price movement. Buys when market is trending up, sells when trending down.',
            howItWorks: [
        ...
        },
        {
            ...
            description: 'Automatically mirrors (or inverses) trades made by specified whale wallet addresses on Polymarket. Polls the data API for new whale trades and replicates them with full risk management.',
            longDescription: 'This strategy watches one or more whale wallet addresses and copies their trades in real-time. ' +
        ...
        },
        {
            ...
            description: 'A blank framework for implementing your own trading logic. Provides all the hooks (market data, order submission, position management) \u2014 you supply the logic.',
            howItWorks: [
        ...
        },
        {
            ...
            description: 'Enters high-probability prediction markets ONLY when market microstructure supports a favorable risk/return profile. Uses 7 cascading filters to reject bad setups, then sizes conservatively via a composite Setup Score.',
            longDescription: 'This strategy targets markets where the implied probability of the leading outcome is between 65\u201396% (configurable) and the market is likely to converge further toward resolution \u2014 or at least not mean-revert against us. ' +
...
class DashboardServer {
    constructor(walletManager, port = 3000) {
        this.walletManager = walletManager;
        this.port = port;
        this.sseClients = new Set();
        this.walletDisplayNames = new Map();
    }
    setWhaleApi(api) {
        this.whaleApi = api;
    }
    ...
    getCopyTradeInstances() {
        ...
        return this.engine.getStrategiesByName('copy_trade')
            .filter((s) => s instanceof copy_trade_strategy_1.CopyTradeStrategy);
    }
    start() {
        if (this.server)
            return;
        this.server = http_1.default.createServer(async (req, res) => {
            ...
            try {
                ...
                logs_1.logger.error({ err: msg }, 'Dashboard request error');
                json(res, 500, { ok: false, error: 'Internal server error' });
            }
        });
        this.server.listen(this.port, () => {
            logs_1.logger.info({ port: this.port, url: `http://localhost:${this.port}/dashboard` }, 'Dashboard server listening');
        });
        // Broadcast dashboard data to SSE clients every second
        this.sseInterval = setInterval(() => {
            if (this.sseClients.size === 0)
                return;
            const payload = (0, dashboard_api_1.buildDashboardPayload)(this.walletManager.listWallets(), this.walletManager.getAllTradeHistories(), this.getLiveMarketPrices(), this.engine?.getPausedWallets(), this.walletDisplayNames);
    ...
    async route(req, res, url) {
        ...
        if (path === '/api/data' && method === 'GET') {
            json(res, 200, (0, dashboard_api_1.buildDashboardPayload)(this.walletManager.listWallets(), this.walletManager.getAllTradeHistories(), this.getLiveMarketPrices(), this.engine?.getPausedWallets(), this.walletDisplayNames));
            return;
        }
        /* ─── JSON: wallet list ─── */
        ...
        if (path === '/api/wallets' && method === 'POST') {
            ...
            const knownStrategies = (0, registry_1.listStrategies)();
            ...
            const wallet = new paper_wallet_1.PaperWallet({
                id: walletId,
        ...
        if (path.match(/^\/api\/wallets\/[^/]+$/) && method === 'PATCH') {
            ...
            if (typeof body.displayName === 'string') {
                ...
                if (name) {
                    ...
                    if (typeof wallet.setDisplayName === 'function')
                        wallet.setDisplayName(name);
        ...
        if (path === '/api/copy-trade/whales' && method === 'GET') {
            ...
            const whalePerformance = addrs.map((a) => {
        ...
        if (path === '/api/copy-trade/whales' && method === 'POST') {
            ...
            for (const inst of instances) {
                if (inst.addWhaleAddress(address))
                    added = true;
            }
            if (added) {
        ...
        if (path.startsWith('/api/copy-trade/whales/') && method === 'DELETE') {
            ...
            let removed = false;
            for (const inst of instances) {
                if (inst.removeWhaleAddress(address))
                    removed = true;
            }
            if (removed) {
        ...
        if (path === '/api/trades/all' && method === 'GET') {
            ...
            for (const [walletId, trades] of allTradesMap) {
                ...
                const walletName = this.walletDisplayNames.get(walletId) ?? ws?.assignedStrategy ?? walletId;
        ...
        if (path === '/api/markets' && method === 'GET') {
            try {
                const fetcher = new market_fetcher_1.MarketFetcher();
        ...
        if (path === '/api/console/stream' && method === 'GET') {
            console_log_1.consoleLog.addSSEClient(res);
        ...
        if (path === '/api/console/logs' && method === 'GET') {
            ...
            json(res, 200, console_log_1.consoleLog.getEntries(limit, offset));
            return;
        }
        if (path === '/api/console/stats' && method === 'GET') {
            json(res, 200, console_log_1.consoleLog.getStats());
            return;
        }
