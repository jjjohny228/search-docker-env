...
function computePerformance(wallet, trades, unrealizedPnl) {
    ...
    const profitFactor = losses.length > 0 && totalLossPnl !== 0
    ...
    const sharpeLike = Number((totalPnl / Math.max(1, wallet.capitalAllocated)).toFixed(4));
    return {
...
function buildDashboardPayload(wallets, tradesByWallet, marketPrices, pausedWallets, displayNames) {
    const entries = wallets.map((w) => {
        ...
        const positions = w.openPositions.filter((p) => p.size > 0).map((p) => {
            ...
            const unrealizedPnl = p.size > 0 && p.avgPrice > 0
