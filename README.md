...
Пример отдельного режима для всех образов конкретного пользователя:
...
docker compose run -d --rm scanner trading --max-results 150 --workers 1 --insecure
```

## Аргументы

- `query` — поисковое слово для Docker Hub.
- `--user-images` — отдельный режим: собрать и проверить все репозитории конкретного Docker Hub пользователя или namespace.
